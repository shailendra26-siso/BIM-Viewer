...
export const executeCore = async (action: Action, events: Events) => {
  ...
  if (action.type === "UPDATE_MODEL") {
    ...
    return databaseHandler.updateModel(id, building, events);
  }
  if (action.type === "DELETE_MODEL") {
    const { model, building, navigate } = action.payload;
    return databaseHandler.deleteModel(model, building, events, navigate);
  }
